...
> (všech 16 osobních + 8 firemních položek) k doslovnému zkopírování je v
...
> - `sidebar-menu.html` – otevřené menu (220 px, celé levé menu 16 + 8 položek, inline SVG ikony, aktivní stav, badge) – generuj JEN na výslovné zadání („otevřené menu“, „rozbalený sidebar“, „menu s texty“)
...
| Moje školení | blok `showSkolMain` (menu `skol`), BEZ vlastní `c800` (defaultní modrá) – design 1:1 podle dodaného standalone prototypu: JEDNA plynulá stránka (dashboard → K vyplnění → Vyplněná → dekorativní „Procházet"), bez drilldownu do kategorie/tagu. Běh vyplňování (`skolMineRun`) i obrazovka „Školení dokončeno" (`skolMineDone`) jsou IN-PAGE náhrady dashboardu, NE modal – viz routing níže |
| Katalog školení | prázdný placeholder (menu `skolc`) – správa a editace školení je záměrně přesunuta do samostatné položky „Správa školení" |
| Správa školení | blok `showSkolAdmin` (menu `skolsprava`) – seznam jako `sc-raw-table` (8 sloupců, sticky první sloupec) design 1:1 podle standalone prototypu; „Přidat školení" (`skolFormOpen`) a „Nastavení školení" (`skolSettingsOpen`) jsou IN-PAGE stránky s header-bar akcemi (ne modal), vzájemně se vylučující se seznamem (`showSkolAdmin` vyžaduje `!skolFormOpen && !skolSettingsOpen`); položka v seznamu otevírá item-drawer `drawerSkolOpen` (vzor Zaměstnanec, 5 tabů) – primární akce (Editovat formulář / Publikovat / Sdílet zaměstnancům / Archivovat) jsou pilulky V HLAVIČCE draweru pod titulkem (`skol_itemActions`), NE v pravém sloupci (ten je jen dekorativní: Zabalit/Historie změn/Duplikovat formulář + metadata); zahrnuje i Návrhář (`skolDesignOpen`) design 1:1 podle standalone prototypu – NENÍ fullscreen, ale zatmavené pozadí (`rgba(30,27,46,.4)`) s plovoucí kartou (padding `12px 12px 12px 34px`); horní lišta „Online formuláře" + X zavírá (`skolCancelDesign`); podtrhávané taby Návrhář/Vyzkoušet/Vzhled/Logika a podmínky + oddělený tab JSON + ULOŽIT (`skolSaveDesign`) v jedné liště; záložka Návrhář má 4 sloupce – hledací paleta 18 položek (`skol_designPalette`), canvas s kartami otázek `skol_designCards` (10 typů inline náhledu: text/comment/choice/checkbox/yesno/rating/video/image/file/signature), panel vlastností (název question{n}, textarea textu, `<select>` typu `skol_designTypeOptions`, toggle-switch Povinná odpověď, možnosti), dekorativní 52px ikonová lišta vpravo; záložka Vyzkoušet sdílí STEJNÁ data `skol_designCards` jako Návrhář |
...
| Moje směrnice (mobil) | obrazovky *smernice* (dlaždice kategorií + rozbalovací „Potvrzeno", stejný princip jako desktop `toggleSmernConfirmed`) / *smernice-detail* („K potvrzení" – plný seznam, funkční POTVRDIT přesouvá dokument do Potvrzeno) |
| Moje školení (mobil) | obrazovka *skoleni* – stejný vzor jako Moje směrnice (souhrnná karta, rozbalovací „Dokončeno", dlaždice kategorií); bez funkčního průběhu vyplňování (mimo rozsah mobilního promatu) |
| Směrnice a dokumenty (mobil) | obrazovky *dokumenty* (obecná knihovna dokumentů – dlaždice kategorií, BEZ potvrzování/procent, na rozdíl od Moje směrnice) / *dokumenty-detail* (dokumenty v kategorii + OTEVŘÍT, tlačítko „Zpět na kategorie") |
