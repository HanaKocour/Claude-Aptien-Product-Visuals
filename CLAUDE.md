...
| Správa školení | blok `showSkolAdmin` (menu `skolsprava`) – staví na vzoru Nastavení směrnic; položka v seznamu otevírá item-drawer `drawerSkolOpen` (vzor Zaměstnanec, 5 tabů); zahrnuje i Návrhář (`skolDesignOpen`, fullscreen editor otázek formuláře) a Nastavení školení (`skolSettingsOpen`, modal Kategorie/Zařazení) |
